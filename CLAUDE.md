...
Веб-приложение «колесо фортуны»: пользователь выбирает настроение, колесо случайно выдаёт что посмотреть или во что поиграть из его собственных списков. Без ручного выбора — только рандом по своим источникам.
...
Два источника контента, в каждом — личный список пользователя:

- 🎬 **Фильмы** — Letterboxd watchlist (из CSV-экспорта)
- 🎮 **Игры** — Steam wishlist (через serverless-прокси из-за CORS)

> Музыка (YouTube playlist) была реализована, но удалена — без хорошего источника метаданных треков mood-фильтр для неё всё равно не работал. Возможные альтернативы (Spotify, Last.fm, Discogs) — на будущее.
